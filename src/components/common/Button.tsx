...
export default function Button({
...
}: { children: React.ReactNode; onClick?: ()=>void; disabled?: boolean; variant?: Variant; className?: string; title?: string }) {
  ...
  return (
    <button title={title} className={`${base} ${variants[variant]} ${className}`} onClick={disabled?undefined:onClick} disabled={disabled}>
      {children}
    </button>
  );
}
