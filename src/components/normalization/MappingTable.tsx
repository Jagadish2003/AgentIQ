...
export default function MappingTable() {
  ...
  useEffect(() => {
    ...
    const selectedExistsOnPage = pageRows.some(row => row.id === selectedRowId);
    if (!selectedExistsOnPage) setSelectedRowId(pageRows[0].id);
    ...
    <div className="flex h-full flex-col rounded-xl border border-border bg-panel p-4">
        ...
        <div className="grid grid-cols-[minmax(0,1.8fr)_minmax(220px,1fr)] gap-3">
          ...
          <div className="relative" ref={sortRef}>
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`shrink-0 text-muted transition-transform ${sortOpen ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  <div className="truncate text-text">{row.sourceSystem}.{row.sourceField}</div>
                  <div className="mt-0.5 truncate text-xs text-muted">{row.commonEntity}</div>
                </div>
                <div className="text-muted">{row.sourceType}</div>
                <div className="text-center text-muted">→</div>
                <div className="truncate text-text">{row.commonField}</div>
                <div>{pill(row.confidence)}</div>
              </div>
            );
          })}
      ...
      <div className="mt-3 flex items-center gap-3 text-sm text-text">
        <button
          ...
          className="rounded border border-border px-2 disabled:cursor-not-allowed disabled:opacity-40"
        >
          «
        </button>
        <button
