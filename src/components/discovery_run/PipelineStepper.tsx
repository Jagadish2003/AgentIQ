...
export default function PipelineStepper({ steps }: { steps: RunStep[] }) {
  return (
    <div className="flex flex-col rounded-xl border border-border bg-panel p-5" style={{ minHeight: '100%' }}>
      <div className="text-sm font-semibold text-text mb-4">Pipeline Progress</div>
      <div className="flex flex-col justify-between flex-1 gap-1">
        {steps.map(s => (
          <div
            key={s.id}
            className="flex items-center justify-between gap-3 rounded-lg px-3 py-3 hover:bg-white/5 transition-colors"
          >
            <div className="flex items-center gap-3">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border text-sm ${
                s.status === 'DONE'    ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300' :
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted mt-0.5">{s.status}</div>
              </div>
            </div>
            <div className={`shrink-0 rounded-md border px-2.5 py-1 text-xs font-medium ${
              s.status === 'DONE'    ? 'border-emerald-500/30 text-emerald-300' :
